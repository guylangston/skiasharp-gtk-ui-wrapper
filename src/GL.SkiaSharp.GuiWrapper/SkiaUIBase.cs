...
public abstract class SkiaUIBase : IUIElement
{
    ...
    /// <summary>Start the application, start the frame timer, drawing each with Skia</summary>
    public int Run()
    {
        Application.Init();
        Window = InitWindow();
        ...
        drawingArea.Drawn += (o, args) =>
        {
            ...
            // GL: I am not sure that this is the fastest/best/most efficient way to draw to the window
            using var img = surface.Snapshot();
            using var data = img.Encode(SKEncodedImageFormat.Png, 100);
            using var stream = new MemoryStream(data.ToArray());
            var pixbuf = new Gdk.Pixbuf(stream);
            Gdk.CairoHelper.SetSourcePixbuf(args.Cr, pixbuf, 0, 0);
            args.Cr.Paint();
            Frame++;
        };
    ...
    protected virtual void OnMainWindowClose() => Application.Quit();
